...
const ProjectSelector: React.FC<ProjectSelectorProps> = ({ type }) => {
    ...
    const projects = useProjectStore(state => state.projects.filter(p => p.type === type));
    ...
    return (
        <div className="relative w-full" ref={dropdownRef}>
            ...
            >
                <span className="font-medium truncate">{selectedProject ? selectedProject.name : t("project.select.title")}</span>
                <ChevronDown size={20} className={clsx("transform transition-transform flex-shrink-0 ml-2", isOpen ? "rotate-180" : "")} />
            </button>

            {isOpen && (
                <div
                    className="absolute top-full left-0 mt-2 w-full rounded-lg shadow-lg 
                ...
                >
                    {projects.length > 0 ? (
                        ...
                        <div className="py-2 px-4 text-sm text-gray-500 dark:text-gray-400">{t("project.select.empty")}</div>
                    )}
