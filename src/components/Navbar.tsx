...
const Navbar: React.FC<NavbarProps> = ({ isExpanded, color, onToggle }) => {
    ...
    return (
        <nav className={clsx("flex flex-col h-full transition-all duration-300", color, isExpanded ? "w-64" : "w-20")}>
            ...
            <div className="flex-1 overflow-y-auto">
                <NavItems isExpanded={isExpanded} appType={appType} projectId={projectId} />
            </div>
        </nav>
    );
};
